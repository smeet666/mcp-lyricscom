...
1. Bump the version in **three** places, which must stay in step:
   ...
   - `src/version.ts`, which feeds the User-Agent sent to lyrics.com
...
```bash
npm view mcp-lyricscom version
npx -y mcp-lyricscom          # should start and wait on stdin
```
