...
Open an issue: <https://github.com/smeet666/mcp-lyricscom/issues>
...
a phrase search that returns a song not containing it is a real bug, and it is only visible to
...
It reads lyrics.com and returns what it reads. It does not write anything back, it
...
- **Every result carries its source.** Content that came from lyrics.com goes out
  with a link to the page it came from, and the text block keeps that link even
  when it has to be shortened.
- **The server paces itself.** lyrics.com is someone else's site. The minimum
...
`LYRICSCOM_LIVE=1 npm run test:live`, which does: it makes one request per route
against https://www.lyrics.com and checks the fields the parsers depend on. Run it if you
...
The tools are `search_lyrics`, `search_songs` and `get_lyrics`. A new tool is a bigger conversation than a new field on
